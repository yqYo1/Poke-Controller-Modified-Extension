...
class isContainTemplate_sample(ImageProcPythonCommand):
    ...
    def do(self):
        ...
        tkmsg.showinfo(
            "確認",
            '先ほどの画像認識は画像をグレースケール化して処理しましたが、パラメータ(use_gray)を設定することで、カラーの状態でも処理できます。\n\n実行関数と結果は右側のログ画面に出力しています。\n\nよければ"OK"を押して下さい。',
        )
        ...
        res = self.isContainTemplate(
            "Samples/logo_pokemon_home.png", use_gray=False, threshold=0.9
        )
        print(f"結果(True:検出成功, False:検出失敗):{res}")
        tkmsg.showinfo(
            "確認",
            '画像認識の検出成功/失敗は類似度(0-1)が閾値を超えるか否かで判定しています。\n\n閾値を超えると検出成功で、閾値はパラメータ(threshold)で変更することが可能です。\n\n実行関数と結果は右側のログ画面に出力しています。\n\nよければ"OK"を押して下さい。',
        )
        ...
        tkmsg.showinfo(
            "確認",
            '類似度はパラメータ(show_value)により表示することが可能です。\n\n実行関数と結果は右側のログ画面に出力しています。\n\nよければ"OK"を押して下さい。',
        )
