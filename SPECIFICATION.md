...
このカテゴリには、CLI側のconfig/profile/source/runtimeセレクターおよび対応するブートストラップ環境変数（`POKECON_*` セレクター）が含まれる（具体的なフラグ名・変数名はCLIスキーマ設計で別途決定する）。ブートストラップ解析で得られた値は、設定パイプライン全体の起点として使用される—後続のパイプライン適用より優先されるわけではなく、「何を読み込むか」を決定するための事前処理である。ブートストラップCLI引数とブートストラップ環境変数の両方が同じセレクターを指定する場合、CLI引数が優先される（後勝ちの一貫性に従う）。

**実行時点による設定変更の分類**:

`pokecon.opt.*`代入が起動パイプラインに属するかランタイムミューテーションであるかは、コードの実行時点に基づいて判定する。コールバックの登録自体は代入を実行しないため、分類の根拠とはならない。実際に代入が実行された時点がパイプライン完了前か後かによってのみ区別される。

- **パイプライン完了前の代入（同期的トップレベル評価中）**: 動的設定の即時評価中に実行された代入は、それがコールバック内で発生した場合でも起動パイプラインに属する。例えば、トップレベルコードがイベントを発行し、そのハンドラが同期的に呼び出されて`pokecon.opt.*`に代入した場合、その代入はパイプラインの一部（パイプライン内順位5）として扱われる。
- **パイプライン完了後の代入**: 起動設定パイプラインが完了し、アプリケーションが通常動作に入った後に実行される代入は、現在の設定状態に対するランタイムミューテーションである。この場合、以下の特性を持つ:
  - `pokecon.opt.*`代入は、その値の由来にかかわらず、現在のインメモリ値を変更する
  - CLI引数由来の現在値であっても変更される可能性がある
  - 変更された値は現在のインメモリランタイム値となり、§11.5.3の永続化ルール（メモリ上のみ）に従う

**CLI引数との相互作用**: 通常CLI引数（順位6）は、パイプライン完了前に実行されたすべての動的設定代入（コールバック経由を含む）の後に適用されるため、それらを上書きできる。一方、パイプライン完了後に実行された代入はCLI引数の後に来るため、CLI由来の現在値を変更できる。
...
- **パイプライン完了後のランタイムミューテーション**: 起動パイプライン完了後に実行される`pokecon.opt.*`代入（コールバック経由を含む）は起動時優先順位の対象外であり、CLI引数由来の現在値を含む任意の現在値を変更できるランタイムミューテーションとして扱われる。パイプライン完了前に同期的に実行されたコールバック内の代入は起動パイプラインに属する（§11.3「実行時点による設定変更の分類」参照）
