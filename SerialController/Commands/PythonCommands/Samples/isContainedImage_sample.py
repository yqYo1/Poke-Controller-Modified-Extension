...
class isContainTemplate_sample(ImageProcPythonCommand):
    ...
    def do(self):
        ...
        self.saveCapture(os.path.join(os.path.dirname(__file__), "capture_image"))
        res = self.isContainedImage(os.path.join(os.path.dirname(__file__), "capture_image.png"), crop=[267, 112, 825, 490], crop_template=[944, 224, 1141, 414], show_only_true_rect=False, show_image=True)
        print(f"検出結果:{res}")
