...
製品要件の正本は[`SPECIFICATION.md`](SPECIFICATION.md)を入口とするbackend／frontend／integrationの三定義書です。本レビューは実装architectureの検討記録であり、UIの相対優先度やframework固有の表現は三定義書の現行記述に合わせます。
...
| Web UI | Browser frontendで製品操作を提供する | 必要な機能要件を満たすfrontend mode。native UIより相対的に下位とはしない |
| Tauri UI | 現行Web frontendをdesktop shellへ表示する | GPUIを含む他frontendと同じbackend機能を提供し、Webより下位の製品機能とはしない |
| GPUI UI | native desktop frontendを追加する | Web/Tauriと同じ製品操作と安全要件を提供する目標。widget・pixel-level visual parityは要求しない。現時点では未実装 |
...
| PWA | 将来のbrowser配布形態として評価する | 独立したbackendや必須の配布方式ではない |
| Theme | UIの色・表示を調整する | frontend機能要件。具体的なCSS/frameworkは固定しない |
| Hardware control | 自動実行・手動操作でcontrollerを制御する | backendが所有する製品の中核機能 |
...
backend定義書と連携定義書は、OS上の監督・資源所有と製品機能上の主従を区別しています。
...
現行Tauriとaxumは同じRustメインプロセスで動作し、デスクトップモードとWebモードは同じ実行ファイルの起動方法として分かれます。GPUI native modeも同一backend ownerを使う目標であり、Nixからのselector追加は未実装です。

frontend mode間に相対的な製品機能優先度は設けません。個々のfrontendは担当する機能・quality要件を満たします。UI制御要求は映像・状態・ログなどの表示配信より優先します。
...
workerのprocess protocolはbackend定義書の必須契約です。現行Web/TauriはHTTP／WebSocket／WebRTCを使用します。GPUI nativeの既存HTTP clientとtyped in-process adapterの選択はPoCで行います。いずれもhardware handleやlanguage runtimeのnative objectをfrontend境界越しに共有しません。
...
動的設定のイベント、コールバック、トランザクション、main側状態は`rust/pokecon/src/dynamic/`が所有し、PyO3とLuaJITの実行実装は`rust/pokecon/src/worker_binary/dynamic/`が所有します。

これらは同じ`pokecon` packageへ統合し、source境界でmain側の動的実行意味論とchild専用runtimeを分けています。

別OSプロセスとして必要な境界は、共通ワーカー実行ファイル、起動時の役割引数、IPCで維持します。
...
Web UIは、操作・監視・停止・復旧を提供するfrontend modeです。

Tauriは現行Web frontendをdesktop shellから提供し、GPUIはnative desktop frontendとして同じbackend機能を提供する目標です。frontend mode間の製品機能優先順位は設けません。

現行Web/Tauriの起動切替を維持し、GPUI nativeは同じPokeCon本体実行ファイルを`nix run .#gpui`から選べる目標です。`--ui gpui`とNix app selectorは未実装であり、追加後もWeb専用の別成果物は作りません。

対応OS向けの標準成果物はWebモードとTauriモードを常に含み、`tauri-shell`の有無による機能差を作りません。

Tauri固有の制約によってWebまたはGPUI frontend、最優先実行環境の機能、低遅延性、安定性を制限しません。どのfrontend modeでも同じbackend ownershipと停止契約を守ります。

最優先実行環境と各frontendの表示配信が資源を競合した場合は、実行環境とfrontendからの制御要求を先に処理します。
...
### 10.11 tool-only devShellと用途別Nix appを併用する

2026年8月9日の利用者指示により、以前の「direnvと既定devShellを廃止する」方針を置き換えます。Nixを利用できる開発hostでは、すべての開発作業をprojectの既定devShell内で行います。

追跡対象の`.envrc`は`use flake`だけを実行し、repositoryへ移動したshellへ既定devShellを自動適用します。新しいworktreeでは`direnv allow`を一度実行し、自動読込みを使わない場合は`nix develop`で同じ環境へ入ります。

既定devShellは固定したRust、Python、Bun、Tauri、品質検査用toolと、対話的なdesktop開発に必要な環境変数を提供します。shellへ入るだけで製品build、依存更新、generator、test、hook導入を実行しません。hostへ直接導入した言語runtime、compiler、package manager、build／品質toolは使用しません。

CI、format、lint、test、build、生成、互換性検査、packageの再現可能な完了操作は、引き続き個別のflake app、`nix fmt`、`nix flake check`が所有します。devShell内で直接実行した対象限定commandは対話的な調査には使えますが、完了証跡にはしません。完了gateはdevShellのambient PATH、cache、設定へ依存せず、必要なtoolと環境を自身で固定します。

次のflake appをdevShellと併用します。
...
`AGENTS.md`、`SPECIFICATION.md`、`PLAN.md`、`README.md`、`docs/DEVELOPMENT.md`、`docs/TROUBLESHOOTING.md`を、tool-only devShellと用途別flake appを併用する記述へ同時に更新します。

direnvが生成する`.direnv/`は非追跡cacheとし、version管理または配布の対象にはしません。

新しい常駐環境、watch、hot reload、書込み、または長時間処理が必要な用途は、既定devShellを肥大化させず、用途と環境を限定したappとして追加します。
...
- Web、Tauri、GPUIを相対優先度のないfrontend modeとし、共通の製品機能とbackend contractを満たす機能境界
...
- 現行Web/Tauri UIを維持し、GPUI native UIを同じPokeCon本体からNix app selectorで追加する構成。selectorは未実装
...
- 追跡した`.envrc`、tool-only既定devShell、用途別flake appを併用し、host toolchainを使用しない構成
...
9. Cargo、frontend dev server、hook導入、エディター連携のflake appを維持し、追跡した`.envrc`とtool-only既定devShellを追加した後、repository全体の開発手順を更新する
...
devShell移行では、新しいworktreeで`direnv allow`または`nix develop`から固定toolchainへ入り、用途別flake appと完了gateを実行できることを確認します。
...
`editor` appまたはNixが出力するlanguage server実行ファイルだけで、Rust、Python、TypeScriptの解析がhost toolchainへ依存せず動作することを確認します。

Git管理対象の`.envrc`が`use flake`だけを含み、文書のdirenv、`nix develop`、devShell参照が同じtool-only開発契約を示すことを確認します。

移行を検証するworktreeで`.direnv/`がGit管理または配布対象になっていないことを確認します。

既存のflake taskをdevShell内と隔離CIから実行し、CI、format、lint、test、build、生成、互換性検査、packageの結果が移行前と一致し、完了gateがambient shell状態へ依存しないことを確認します。
