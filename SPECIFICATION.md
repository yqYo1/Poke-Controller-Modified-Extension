...
| **後勝ち** | 複数の設定ソース間で同じキーが存在する場合、優先順位の高いソースの値を採用する方式。評価/実行順序ではなく、ソース選択の優先順位に基づく |
...
設定値の優先順位は**ソース選択の優先順位**であり、評価/実行順序ではない。複数の設定ソースから同じキーの値が得られた場合、以下の最も高い優先順位のソースの値を採用する。実装はソースの評価順序を任意に設定できるが、採用される値はこの優先順位に従わなければならない。

優先順位（高いほど優先）:

1. **デフォルト値**（アプリケーション内蔵） — 最下位
2. **グローバル設定**（`~/.config/pokecon/settings.toml`）
3. **プロファイル設定**（`~/.config/pokecon/profiles/<name>/settings.toml`）
4. **動的設定**（`~/.config/pokecon/init.py`/`init.lua`） — トップレベルのシンプル代入（`pokecon.opt.foo = value`）に限る
5. **起動時引数**（CLIオプション） — 最優先

```
起動時優先順位（ソース選択）: ①デフォルト → ②グローバル → ③プロファイル → ④動的設定 → ⑤CLI引数
                              （低）                                              （高）
```

**注**: CLI引数は起動時の明示指定であるため、動的設定のトップレベル代入よりも優先される。動的設定ファイルの評価タイミング（CLI解析の前か後か）によらず、CLI引数で指定された値が有効となる。動的設定のトップレベル代入（例: `init.py` 中の `pokecon.opt.foo = value`）は動的設定レイヤーに属し、CLI引数より低い優先順位である。イベントコールバック内での実行時ミューテーションの扱いは本セクションの対象外とし、別途決定する。
