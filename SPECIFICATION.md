...
1. **JSON文字列**: 複合値はすべてJSON文字列として受け渡す。パース後に正準レジストリの型で検証する（本項4参照）。
   ...
   - `python.script.packages.list`（型 `list[{name: str, version?: str}]`）:
     - CLI: `--python-script-packages-list '[{"name":"requests","version":">=2.28.0"}]'`
     - 環境変数: `POKECON_PYTHON_SCRIPT_PACKAGES_LIST='[{"name":"requests","version":">=2.28.0"}]'`
...
3. **キー全体置換**: CLI引数および環境変数による複合値の設定は、**キー全体の置換**として動作する。要素レベルのdeep mergeや部分更新は行わない。この動作は§11.3のマージ方式（キー単位マージ、複合値は値全体を1キーとして扱う）と一貫する。
