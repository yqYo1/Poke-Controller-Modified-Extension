...
**言語仕様**:
- **Python**: 3.14以上をターゲット。ランタイムは3.14を使用するため、3.14で使用可能な記法を必須とする。可能な限り3.13にも存在する記法を使用し、3.14時点で非推奨・廃止予定の機能は使用しない。PEP 695型パラメータ、basedpyrightによる厳格な型チェックを使用
...
> **要件**: PWAは今後のバージョンで実装予定です。詳細は§17.1を参照。
