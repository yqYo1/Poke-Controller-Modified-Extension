...
- **不正終了時の挙動**: OKボタン以外でダイアログが閉じられた場合（×ボタン、Escキー、強制終了など）、ユーザースクリプトを停止する
...
- 結果は各Widgetの`value`属性に格納される
- **スクリプト停止時の挙動**: ユーザースクリプトが停止（Stopボタン、エラー、強制終了など）した場合、スクリプト内で作成したすべての非ブロッキングダイアログを自動で閉じる
...
- **不正終了検出**: ダイアログが不正終了（OKボタン以外で閉じられた）場合、`is_dialog_closed`呼び出し時にユーザースクリプトを停止する
...
- **不正終了時の挙動**: OKボタン以外でダイアログが閉じられた場合、ユーザースクリプトを停止する
