...
- `performance-check`は絶対閾値を毎回評価し、同じworkflow event・同じbranch（PRは同じPR）・同じfixture世代で全体がsuccessとなった検証済みpassed reportを最大10件収集します。10件揃った場合だけbaselineとして各metricを集約し、browser loopback fixture本体（`scripts/performance/`）またはruntime入力（`flake.nix`、`flake.lock`）が変更されたrunでrelative regressionを判定します。latencyの相対許容上限は`max(中央値×1.10, 履歴p95最大値)`、FPSの相対許容下限は`min(中央値×0.95, 履歴p50最小値)`です。10件未満はbootstrapとして記録し、relative比較は行いません。これは履歴の実測分散を評価するためで、絶対閾値を緩めるものではありません。本番Rust／Web UI／serial等の変更はこのloopback fixtureを実行時に呼び出さないため、browser smokeと絶対閾値は実行しますが、relative baseline比較は行いません。同一対象commitの別runはbaselineへ再利用しません。
