...
2. 主機能、主機能の成立に不可欠な支援機能、優先度の低い補助機能
...
一方、定義書には、これらのうち何がPokeConの主機能であり、何が主機能を成立させる支援機能であり、何が優先度の低い補助機能かという序列が明示されていません。

調査で確認できた機能群と、定義書上の扱いは次のとおりです。

| 機能群 | 定義書で確認できる扱い | 相対的な優先順位 |
|---|---|---|
| Python／MCU自動化コマンド | 実行制御と既存スクリプト互換性を必須とする | 他機能より主であるかは未定義 |
| カメラと画像処理 | 映像取得、画像処理、スクリーンショットを提供する | 自動化の支援機能か独立した主機能かは未定義 |
| シリアルとコントローラー出力 | ゲーム機への入力、複数形式、手動操作を提供する | 自動化と手動操作のどちらを主とするかは未定義 |
| Web／Tauri UI | 旧Tkinter UIとの機能・視覚的パリティを必須とする | UI自体と自動化機能の優先関係は未定義 |
| 設定、プロファイル、動的設定 | 機能切替、複数用途、実行時カスタマイズを提供する | 主機能との優先関係は未定義 |
| 通知 | Windows通知とDiscord通知を提供する | 他機能との優先関係は未定義 |
| PWA、テーマ、ハードウェア制御 | 今後のバージョンで実装する | 現行機能より低いことが明示されている |

仕様上の「必須」は各機能の受入条件を示しますが、製品内での相対的な重要度までは示しません。
