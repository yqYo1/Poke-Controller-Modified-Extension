...
動的設定の即時評価は、`init.py`/`init.lua`のトップレベルで実行される遅延されていないコードのみを含む。イベントコールバックや`pokecon.autocmd`に登録された遅延コードはこの起動時パイプラインの対象外であり、起動後にイベント駆動で実行される。

**ランタイムミューテーションとしてのイベントコールバック**:

イベントコールバックやその他の遅延動的設定コード内で実行される`pokecon.opt.*`代入は、設定ソースの優先順位解決ではなく、現在の設定状態に対する明示的なランタイムミューテーションである。このような代入は起動時パイプラインを経由せず、以下の特性を持つ:

- コールバック内の`pokecon.opt.*`代入は、その値の由来（CLI引数、環境変数、TOML設定、組み込みデフォルトのいずれか）にかかわらず、現在のインメモリ値を変更する
- CLI引数由来の現在値であっても、後続のコールバック代入によって変更される可能性がある
- 変更された値は現在のインメモリランタイム値となり、既存の動的設定メモリ/永続化ルール（§11.5.3「メモリ上のみ」）に従う — すなわち、TOMLへの永続化や再起動時の保持は行わない
- この区別は起動時のみ有効: トップレベルの即時動的設定代入は通常CLI引数によって上書きされるが（起動時パイプライン内の順序による）、遅延イベントコールバック代入は起動後に実行されるためCLI由来の現在値を変更できる
...
- **イベントコールバック内のランタイムミューテーション**: イベントコールバックや遅延コード内の`pokecon.opt.*`代入は起動時優先順位の対象外であり、CLI引数由来の現在値を含む任意の現在値を変更できるランタイムミューテーションとして扱われる（§11.3「ランタイムミューテーションとしてのイベントコールバック」参照）
