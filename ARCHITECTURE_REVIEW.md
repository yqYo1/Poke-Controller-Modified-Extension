...
同じパッケージの複数の`[[bin]]`で、本体、ユーザースクリプトワーカー、動的設定ワーカー、開発用生成器を提供します。
...
|---|---|---|
| `pokecon-app` | 本体パッケージの統合先 | メインプロセスと製品全体の合成起点を所有する |
...
| `pokecon-dynamic` | 本体へ統合 | 動的設定ワーカーは別`[[bin]]`と内部モジュールで表現できる |
| `pokecon-desktop` | 条件付きで別パッケージを維持 | Tauri固有のビルド、署名、インストーラー、OS依存を隔離する実益を確認できる場合だけ維持する |
...
| `pokecon-worker` | 本体へ統合 | 別OSプロセスは別`[[bin]]`で表現し、IPC契約とプロセス監督を本体ライブラリーで共有できる |

統合後は原則としてPokeCon本体の1パッケージとし、Tauri固有の実益を確認できる場合だけデスクトップパッケージを追加します。
...
動的設定エンジンは本体パッケージへ統合し、別OSプロセスとして必要な境界は動的設定ワーカー用の`[[bin]]`とIPCで維持します。
...
現在の11クレートのうち、`pokecon-pybindings`と`pokecon-desktop`を除く実装はPokeCon本体の1パッケージへ統合し、本体、ユーザースクリプトワーカー、動的設定ワーカー、開発用生成器を複数の`[[bin]]`として配置します。

`pokecon-pybindings`は削除します。

`pokecon-desktop`は、Tauri固有のビルド、署名、インストーラー、OS依存を本体から隔離する実益を確認できる場合に限り、別パッケージとして維持します。
...
- PokeCon本体を1パッケージへ統合し、別OSプロセスを複数の`[[bin]]`として維持する構成
- `pokecon-pybindings`とネイティブwheelを削除する移行手順
- Tauriデスクトップを別パッケージとして維持する場合の実証されたビルド・配布要件
- 製品全体で優先する設計原則
- 正準状態とハードウェアリソースの所有者
- プロセスごとの責務、寿命、再生成規則
