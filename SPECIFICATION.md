...
```text
┌─ Rust監督・資源サービスプロセス ─────────────────────────┐
│ Tauri / axum / 正準状態 / 安全停止 / シリアル / カメラ   │
└──────────────────────────────────────────────────────────┘
                ↕ 双方向IPC                    ↕ 双方向IPC
┌──────────────────────────────┐ ┌──────────────────────────┐
│ ユーザースクリプトワーカー   │ │ 動的設定ワーカー        │
│ 自動実行時の機能上の実行主体 │ │ 実行環境の支援機能      │
└──────────────────────────────┘ └──────────────────────────┘
```

Rustメインからユーザースクリプトワーカーへ流れる起動、コマンド開始要求、停止、世代切替は監督制御である。ユーザースクリプトワーカーからRustメインへ流れるカメラ、シリアル、コントローラー、UI、通知等の要求は自動実行の主制御である。安全停止、全入力解放、ニュートラル送信はこの主制御へ常に割り込める。
