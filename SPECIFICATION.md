...
**注**: ユーザースクリプトや動的設定（`init.py`/`init.lua`）から呼び出されるAPIは、原則としてPyO3（Rust製）で実装される。Pythonファイル（`commands.py`, `events.py`等）は型注釈・ドキュメント・互換レイヤーのみを提供し、実際の処理はRust側で行う。
...
>
> **メニュー項目について**: LINE Token Assignment / LINE Token Check のメニュー項目は、旧UI互換のためメニュー構造に残存します（§11.13.1参照）。ただし、これらのメニュー項目を選択しても何も起こらない（No-op）か、削除済みである旨のメッセージを表示します。
...
- **キーボードショートカット**: キーマップシステム（§11.14）で管理。デフォルトでは`1`～`0`（数字キー）に割り当て（§11.14.5参照）。修飾キー付きのショートカットも可能
...
- **実行制御キーとの関係**: ショートカットボタンはキーマップシステム（`pokecon.keymap.set()` で登録）で管理。実行制御キー（§11.14.5）も同じキーマップシステムで管理。両者は同じシステムであり、同じキーが両方に割り当てられていた場合、後勝ちが適用される。ショートカットボタンはデフォルトで`1`～`0`（数字キー）に割り当て、実行制御キーはデフォルトで未割り当て（§11.14.5参照）
...
キーボードショートカットの割り当ては **§11.14.5 デフォルトキーマップ** を参照。
...
| 品質制御 | 固定品質（CRF）でエンコード。帯域測定と品質段階自動選択は将来の拡張 |
...
> **注**: `cam` と `gui` は互換性維持のためのパラメータ。リファクタリング前のスクリプトでは `cam` を必須、`gui` を省略可能として使用。リファクタリング後はRustコアのカメラ/表示システムに自動的にマッピングされる。**ユーザースクリプトが直接インスタンス化する場合、引数を指定する必要はない（フレームワーク側が自動的に渡す）。ただし、クラス定義上は `cam` が必須引数であり、後方互換性のためこのシグネチャを維持する**
...
- **エラーハンドリング**:
  - **Python**: 構文エラー時はファイル全体の読み込みに失敗し、フォールバック設定を使用
...
- **動詞に限定しない**: 名詞・形容詞も可
**注**: 動的設定用イベントシステム（§11.12）とWebSocketイベント（§7.3.2）は**別々のシステム**です。
...
type KBKeys = Literal[
    ...
    # 特殊キー（例示。§11.14.3.1のテーブルに定義されたすべてのキーが使用可能）
...
**要件**: §11.14.3.1のテーブルに定義されたすべてのキー（`<Nul>`, `<NL>`, `<Bar>`, `<Ignore>`, `<lt>`, `<Bslash>`, `<nop>` 等を含む）は、キーマップシステムで使用可能。
