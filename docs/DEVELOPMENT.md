...
`flake.nix`の構造を変更した場合は、`canonicalFlakeHash`に加えて監査テストの区間ハッシュ定数と正規化ハッシュ定数（`tests/quality/test_ui_package_check.py`）も更新します。監査テスト自体を変更した場合は`flake.nix`の`expectedAuditTestHash`（テストファイルの生ハッシュ）を更新し、続けて`canonicalFlakeHash`を再計算します。整合は`production-routing-audit`gateで検証できます。
