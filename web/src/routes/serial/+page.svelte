<script lang="ts">
	...
	async function handleSend() {
		...
		await api.writeSerial(sendData);
		sendData = '';
	}
</script>
...
<div class="space-y-4">
	...
	<div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
		...
		<div class="rounded border border-gray-700 bg-gray-900 p-3">
			<h3 class="mb-2 text-sm font-medium text-gray-200">データ送信</h3>
			<div class="flex gap-2">
				<input
					bind:value={sendData}
					placeholder="送信データを入力..."
					disabled={!connected}
					class="flex-1 rounded bg-gray-800 px-2 py-1 text-xs text-gray-200 placeholder-gray-500 disabled:opacity-50"
				/>
				<button
					onclick={handleSend}
					disabled={!connected}
