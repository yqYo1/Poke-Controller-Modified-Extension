"""
Compatibility wrapper for Commands.Keys module.

Provides the exact same public API as the original SerialController/Commands/Keys.py
but delegates to (or is designed to integrate with) the Rust pokecon-serial crate.
"""
...
class SendFormat:
    ...
    def setButton(
        ...
        for btn in btns:
            self.format["btn"] |= convert[btn]  # type: ignore[operator]
    ...
    def setHat(self, btns: list[Hat], convert: list[int] = convert_hat_default) -> None:
        if not btns:
            ...
            self.format["hat"] = convert[btns[0]]
    ...
    def setTouchscreen(self, dirs: list[Touchscreen]) -> None:
        if dirs:
            self.format["sx"] = dirs[0].x
            self.format["sy"] = dirs[0].y
...
class KeyPress:
    ...
    def __init__(self, ser) -> None:
        ...
        self.holdLeftStick: bool | Direction = False
    ...
    def holdButtons(self):
        ...
        if self.holdLeftStick:
            hold_btns.append(self.holdLeftStick)
        if self.holdRightStick:
            hold_btns.append(self.holdRightStick)
        if self.holdTouchscreen:
            hold_btns.append(self.holdTouchscreen)
        return hold_btns
    ...
    def input(self, btns, ifPrint: bool = True) -> None:
        ...
        if self.serial_data_format_name == "3DS Controller":
            self.format.setButton(
                ...
                convert=conversion_3ds_controller_button,  # type: ignore[arg-type]
            )
