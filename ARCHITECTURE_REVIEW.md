...
同じパッケージの複数の`[[bin]]`で、PokeCon本体、管理対象ワーカー、開発用生成器を提供します。

ユーザースクリプトワーカーと動的設定ワーカーは同じ管理対象ワーカー実行ファイルを使い、起動時引数で役割を切り替えます。
...
| `pokecon-dynamic` | 本体へ統合 | 動的設定ワーカーは管理対象ワーカー実行ファイルの起動時役割と内部モジュールで表現できる |
...
| `pokecon-worker` | 本体へ統合 | 別OSプロセスは管理対象ワーカー用の`[[bin]]`で表現し、スクリプトと動的設定の役割を起動時引数で切り替えられる |
...
| 種別 | 名前 | 役割 |
|---|---|---|
| 製品実行ファイル | `pokecon` | Web／Tauri切替、メインプロセス、正準状態、ハードウェア所有 |
| 管理対象ワーカー | `pokecon-worker` | `--kind script`／`--kind dynamic`で役割を切り替える別OSプロセス |
...
| `worker` | IPC、世代管理、プロセス監督、スクリプト実行 |
...
動的設定エンジンは本体パッケージへ統合し、別OSプロセスとして必要な境界は共通の管理対象ワーカー実行ファイル、起動時の役割引数、IPCで維持します。
...
現在の11クレートのうち、`pokecon-pybindings`を除く実装はPokeCon本体の1パッケージへ統合し、PokeCon本体、管理対象ワーカー、開発用生成器を複数の`[[bin]]`として配置します。

管理対象ワーカーは、起動時引数でユーザースクリプトまたは動的設定の役割を選択します。
...
- PokeCon本体を1パッケージへ統合し、共通の管理対象ワーカー実行ファイルを役割引数付きの別OSプロセスとして起動する構成
...
4. `pokecon-dynamic`と`pokecon-worker`を移し、`pokecon-worker --kind script`と`--kind dynamic`の別プロセス境界、IPC、強制停止、世代管理を維持する
