...
class GetFromHomeGUI:
    ...
    def makePokeDetailList(self, values):
        ...
        for i in range(10):
            try:  # 凄く強引な実装なので修正したい…
                ...
                try:
                    beatedWaza[i] = [
                        self.poke_data[values[0]][values[5]]["lose"]["waza"][i]["id"],
                ...
                try:
                    beatWaza[i] = [
                        self.poke_data[values[0]][values[5]]["win"]["waza"][i]["id"],
    ...
    def treeview_sort_column(self, tv, col, reverse):
        try:
            try:
                _l = [(int(tv.set(k, col)), k) for k in tv.get_children("")]
            except Exception:
                _l = [(float(tv.set(k, col)), k) for k in tv.get_children("")]

        except Exception:
            _l = [(tv.set(k, col), k) for k in tv.get_children("")]

        _l.sort(reverse=reverse)

        # rearrange items in sorted positions
        for index, (val, k) in enumerate(_l):
            tv.move(k, "", index)
    ...
    def dl_rank_poke_data(self, isSingle, rst, ts2):
        ...
        path = "db/pokedata_Season" + self.season.get() + "_" + _l + "Battle" + ".json"
