...
| 7.8.4 SerialDisconnected: 未開始シリアル送信の拒否; 実行中は直列化 | 実装済み | `src/script_host.rs:2439-2497` queued `SendCancelled`→`SerialDisconnected` regression; `:2499-2507` disconnected send mapping; `src/script_host.rs:2135-2141` public code mapping; `src/worker/ipc/schema.rs:442-447` closed ASCII code validation |
...
| 7.9.2 構成: 名前付き永続SHM、両OS同一意味 | 部分的 | `src/camera/shared_ring.rs:339-350` timeout fallbackのPOSIX name unlink／Windows no separate unlink; `src/camera/shared_ring.rs:662-671` `shm_unlink`（既存mappingは保持）; `src/camera/manager.rs:120-133,319-359,365-366,402-453` writer timeoutからfallbackへ接続。shm_open/CreateFileMapping直接シンボルなし |
