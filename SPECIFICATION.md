...
本マッピングは、三つの不変ベースラインリポジトリ（§4.6.1参照）の**必須初期依存関係セットの積集合**として定義される。ほとんどのユーザースクリプトは三環境すべてで動作するよう設計されているため、初期ワーカースクリプト必須セット（`worker-script`）には三ベースラインすべてに共通するパッケージのみを含める。制約バージョンは、ベースラインマニフェストに実際に存在する最も強い互換下限を使用する（上限なしの要件はunboundedとする）。

**`worker-script`（ユーザースクリプトワーカー） — 初期必須セット（10配布物）**:
...
**非共通パッケージ（初期必須セットから除外）**:

以下のパッケージは三ベースラインのうち一部にのみ存在するか、または過去のアプリケーション実装に由来するものであり、`worker-script`初期必須セットには含めない。これらは必ずしも「廃止」や「非サポート」を意味するものではない。ユーザーは`packages.list`（§14.5.1）を介して明示的に指定できる。将来の互換性昇格スクリプト（§4.6.2）の検証を通過した場合、本マッピングに追加される可能性があるが、初回リリース時点では追加しない。
...
- 10配布物積集合（§14.5のworker-script初期必須セット）にはsetuptoolsは含まれず、三ベースラインすべてに共通するユーザースクリプト要件ではない。
...
| パッケージ | 備考 |
|---|---|
| `deprecated` | 三ベースラインに共通しない |
| `gitpython` | 三ベースラインに共通しない |
| `icecream` | 三ベースラインに共通しない |
| `imagehash` | 三ベースラインに共通しない |
| `loguru` | 三ベースラインに共通しない |
| `numba` | 三ベースラインに共通しない |
| `paho-mqtt` | 三ベースラインに共通しない |
| `plyer` | 三ベースラインに共通しない |
| `pygame` | 三ベースラインに共通しない |
| `pyocr` | 三ベースラインに共通しない |
| `pyyaml` | 三ベースラインに共通しない |
| `setuptools` | 過去のアプリケーション側実装から残存する履歴的依存関係。ユーザースクリプト非依存。ビルドバックエンドはmaturin。10配布物積集合に含まれない。pyproject.tomlの`[project].dependencies`から削除すること（上述の詳細理由参照）。 |
| `windows-capture-device-list` | 三ベースラインに共通しない |
...
**スコープ**: 環境検証（fingerprint再確認）、uv解決・インストール、準備完了マークのコミット、アプリ管理テンポラリファイルのクリーンアップを含む全段階をロック範囲とする。この範囲をロック解放前に終了しなければならない。ロックは正常終了・失敗・キャンセルの全経路で確実に解放する。
