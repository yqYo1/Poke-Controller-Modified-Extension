...
- `performance-check`は絶対閾値を毎回評価し、同じworkflow event・同じbranch（PRは同じPR）・同じfixture世代で全体がsuccessとなった直近最大5件の検証済みpassed reportをbaselineとして各metricのnearest-rank中央値を求めます。product／performance入力が変更されたrunでは、その中央値に対してlatency 10%増加またはFPS 5%低下をregression failureとします。同一対象commitの別runはbaselineへ再利用しません。CI制御だけが変更されたrunはbrowser smokeと絶対閾値を実行しますが、product性能を変更していないためrelative baseline比較は行いません。過去reportがない初回はbootstrapと記録し、regression成功とは扱いません。
