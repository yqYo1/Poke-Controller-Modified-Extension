...
2. §7.9.4の選択規則に従い、書き込み可能なスロット（書き込み中でなく、ワーカーによりピン留めされていないスロット）を選択する。カメラバックエンドが共有メモリへの直接キャプチャをサポートする場合はそれを優先し、サポートしない場合はキャプチャバッファから選択スロットへの1回のメモリコピーを行う。
3. フレームのバイト列とメタデータ（フレーム識別子、形状、ストライド、データ型）を選択スロットに完全に書き込む。
4. Rustメインは共有ヘッダ（§7.9.3 SharedHeader）の `latest_slot_index` と `latest_frame_id` を**リリースセマンティクス**でアトミックに更新し、当該フレームを読取可能として出版する。
5. ワーカーは共有ヘッダを必要なタイミングで**獲得セマンティクス**で読み取り、最新の `(slot_index, frame_id)` 組を取得する（ワーカーは§7.8制御IPC経由のフレーム通知を個別に受信しない）。
...
| フィールド | 型 | 説明 |
|-----------|-----|------|
| `latest_slot_index` | アトミックuint32 | 最後に出版完了したフレームが格納されているスロット番号（0..2） |
| `latest_frame_id` | アトミックuint64 | 単調増加フレーム識別子。出版のたびに増加 |

Rustメイン（writer）はフレーム書き込み完了後に上記2フィールドをリリース順序で更新する。ワーカー（reader）は獲得順序で読み取る。

**SlotHeader**（各スロットの先頭、3スロット分）:

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `frame_id` | アトミックuint64 | このスロットに最後に書き込まれたフレームの識別子。shared_header.latest_frame_idと比較し整合性確認に使用 |
...
> **注**: MappingDescriptorの正確なフィールドセットとシリアライゼーション形式は実装時に決定する。上記は最小限の契約フィールドである。
...
1. カメラキャプチャ後、writerは以下の条件をすべて満たすスロットを選択する:
   - `state` が `0`（空/書込可能）である
   - `reader_pin_count == 0`（ワーカーにピン留めされていない）
2. 条件を満たすスロットがない場合（全3スロットが書込中/出版済/ピン留め中）:
   - キャプチャをブロックしない。新たにキャプチャされたフレームを**ドロップ**する。
   - 既存の読取側の最新フレームは引き続き有効であり、無効化されない。
3. 選択したスロットの `state` を `1`（書込中）に設定する。
4. カメラバックエンドが共有メモリへの直接キャプチャをサポートする場合はそれを優先する。サポートしない場合、キャプチャバッファから選択スロットへの1回のメモリコピーを行う。
5. フレームバイト列およびメタデータ（frame_id, shape, strides, dtype）を完全に書き込む。
6. `state` を `2`（出版済）に設定し、SharedHeaderの `latest_slot_index` / `latest_frame_id` をリリースセマンティクスで更新する。

##### 読取手順（reader / ワーカー）

1. Camera API呼び出し時、SharedHeaderの `latest_slot_index` と `latest_frame_id` を**獲得セマンティクス**でアトミック読み取りする。
2. 該当スロットの `reader_pin_count` をインクリメントする。
3. SharedHeaderを再読取し、`(slot_index, frame_id)` が変化していないことを確認する。
4. 変化していた場合（レース敗北）、ピンを解放し手順1から再試行する。
5. 安定したスロットからワーカー内プライベートmutable ndarrayへコピーする。
6. コピー完了後、直ちに `reader_pin_count` をデクリメントする。
7. 呼出元に返されるndarrayはワーカープライベートメモリ上の独立コピーであり、共有スロットをピン留めしない。
...
- ワーカーのIPCが切断された場合（EOF）またはワーカープロセスが異常終了した場合、Rustメインは:
  ...
  2. 共有メモリ領域のマッピングを解除し、名前付き共有メモリを解放する。
- ワーカー側の明示的な後処理は不要。
