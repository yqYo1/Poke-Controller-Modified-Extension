...
- 現在の検証済み修正: **111 / 455 finding、40 / 139 finding-file**（Wave 1/2を継続中、Wave 3はCI timing／normal-ciの5件を完了）。
- Wave 1: **98 / 222 finding、33ファイル**。Wave 2: **8 / 110 finding、5ファイル**。Wave 3: **5 / 123 finding、2ファイル（timing.py、normal-ci.yml完了）**。
...
|---|---|---:|---:|---|---|
| Wave 1 | Rust runtime、settings、worker、server、contracts、Rust tests | 65 | 222 | 部分完了（98/222） | realtime_connection focused compile/test passed; signed commits continue |
...
| 137 | `rust/pokecon/src/worker_binary/dynamic/runtime/python.rs` | 7 | high,high,high,medium,medium,medium,medium | Wave 1 | 一部修正・検証済み | F1をtrusted-code契約として`docs/DYNAMIC_CONFIGURATION.md`へ明示、F3を`e649bfd`、F4を`77d7c25`で修正。Python source evaluationを`spawn_blocking`へ移し、EvaluationScopeを伝搬、coordinatorを評価中に解放、generation再検証、Python monitoring tool idの非強制割当を追加。adversarial coordinator test、engine 8 tests、full lib 454 passed、contract-check成功。F2/F5/F6/F7は未着手。 |
