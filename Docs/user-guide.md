...
| タブ | 用途 | スマホ操作 |
|------|------|----------|
| **📷 Camera** | カメラプレビュー、デバイス設定、キャプチャ | タップで操作 |
| **🔌 Serial** | シリアルポート接続管理、送受信 | フォーム入力 |
| **🎮 Manual** | 仮想ゲームパッド、マウススティック操作 | タップ・スワイプで入力 |
| **📜 Commands** | スクリプト一覧と実行管理 | タップで開始/停止 |
| **⌨️ Keyconfig** | キーボード→ボタン割り当て設定 | フォーム入力 |
| **🏠 Pokémon Home** | ボックスデータ管理 | タップ操作 |
| **🔔 Notification** | Discord/LINE通知設定 | フォーム入力 |
| **⚙️ Others** | プロファイル、テーマ、マウス設定 | フォーム入力 |
...
- **カメラプレビュー**: リアルタイムでゲーム画面を表示
  - **WebSocketプッシュ**: サーバーからフレームがWebSocket経由でプッシュされます（デフォルト、~30fps）
  - **WebRTC**: WebRTC DataChannelを使用した低遅延ストリーミング（オプション、SDPネゴシエーションはWebSocketを介して自動実行）
  - **MJPEGストリーム**: `<img>` タグで `/api/camera/stream.mjpeg` を指定したtrue streaming（Content-Type: multipart/x-mixed-replace）
  - 回線状況に応じて自動フォールバック: WebRTC → WebSocket → HTTP polling
