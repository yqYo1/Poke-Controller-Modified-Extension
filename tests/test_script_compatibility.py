...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(__file__)), "SerialController"))
...
class TestPythonCommandImports:
    ...
    def test_image_proc_command_has_required_methods(self) -> None:
        ...
        for method_name in required_methods:
            assert hasattr(ImageProcPythonCommand, method_name), f"Missing method: {method_name}"
...
class TestSampleInstantiation:
    ...
    def test_is_contain_template_sample_instantiation(self, mock_camera: MockCamera) -> None:
    ...
    def test_is_contained_image_sample_instantiation(self, mock_camera: MockCamera) -> None:
...
class TestPublicApiWithMocks:
    ...
    def test_image_proc_is_contain_template(self, mock_camera: MockCamera, tmp_path) -> None:
        import cv2
        cmd = ImageProcTest(mock_camera)
        ...
        with patch("ImageProcessing.ImageProcessing") as mock_img_proc:
            ...
            result = cmd.isContainTemplate("dummy.png", threshold=0.7, use_gray=True, show_position=False)
            assert result is True

    def test_image_proc_is_contained_image(self, mock_camera: MockCamera, tmp_path) -> None:
        import cv2
        cmd = ImageProcTest(mock_camera)
        dummy_img = np.zeros((100, 100, 3), dtype=np.uint8)
        ...
        with patch("ImageProcessing.ImageProcessing") as mock_img_proc:
            ...
            result = cmd.isContainedImage("dummy.png", threshold=0.7, show_position=False)
            assert result is True
