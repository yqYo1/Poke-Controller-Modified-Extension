...
/// every import used by a fixed baseline script is classified as either a
/// bundled API or an externally distributed dependency.
///
/// # Errors
///
...
fn validate_public_surface() -> Result<(), ContractError> {
    ...
    let expected_imports = BTreeMap::from([
        ("Commands._meta", strings(&["CommandMeta"])),
        ("Commands.CommandBase", strings(&["Command"])),
        (
            "Commands.Keys",
            strings(&[
                "Button",
    ...
    require_equal(
        "user-script external imports",
        &surface.external_imports,
        &vec![ExternalImport {
            module: "Commands.PythonCommands.bridge_functions.bridge_functions".to_owned(),
            symbols: strings(&["BridgeFunctions"]),
            bundled: false,
            reason: "separate_license_and_distribution".to_owned(),
            source: "compatibility/fixed-manifest.json".to_owned(),
        }],
    )?;

    let expected_classes = BTreeMap::from([
        ("CommandMeta".to_owned(), Vec::new()),
        ("Command".to_owned(), strings(&["isRunning"])),
        (
            "Camera".to_owned(),
            strings(&[
                "image_bgr",
...
fn validate_fixed_imports() -> Result<(), ContractError> {
    ...
    let surface = registry
        ...
        .find(|surface| surface.worker == "user_script")
        .ok_or_else(|| invariant("protocol registry has no user_script public surface"))?;
    let mut classified = BTreeMap::<&str, BTreeSet<&str>>::new();
    for required in &surface.required_imports {
        classified
            .entry(required.module.as_str())
            .or_default()
            .extend(required.symbols.iter().map(String::as_str));
    }
    for external in &surface.external_imports {
        classified
            .entry(external.module.as_str())
            .or_default()
            .extend(external.symbols.iter().map(String::as_str));
    }
    for (module, names) in fixed_imports {
        let available = classified.get(module.as_str()).ok_or_else(|| {
            invariant(format!(
...
const COMMANDS_TYPING_FILES: &[PythonTypingTemplate] = &[
    ...
    PythonTypingTemplate {
        relative_path: "Commands/_meta.pyi",
        source: COMMAND_META,
    },
    PythonTypingTemplate {
        relative_path: "Commands/CommandBase.pyi",
        source: COMMAND_BASE,
    },
    PythonTypingTemplate {
...
const COMMAND_META: &str = r"# Generated by `nix run .#generate-contracts`; do not edit.
class CommandMeta(type): ...
";

const COMMAND_BASE: &str = r"# Generated by `nix run .#generate-contracts`; do not edit.
...
class PythonCommand(Command, metaclass=CommandMeta):
    _logger: Logger
    keys: KeyPress
...
class ImageProcPythonCommand(PythonCommand, metaclass=CommandMeta):
    camera: Camera
    cam: Camera
    gui: CaptureArea
    canvas: CaptureArea
...
class McuCommand(Command, metaclass=CommandMeta):
...
mod tests {
    ...
    fn generated_typings_cover_the_canonical_surface() {
        ...
        assert!(!combined.contains("BridgeFunctions"));
        assert!(
            files
                .iter()
                .all(|file| !file.relative_path().contains("bridge_functions"))
        );
    }
