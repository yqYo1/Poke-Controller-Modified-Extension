...
- [x] **AR-11-25** 正準controller状態とcamera／serial handle等のhardware resource所有者を一意にする（2026-09-27、ownership table（server socket・worker世代・設定revisionを含む。`docs/ARCHITECTURE_HANDOFF.md`§3）と重複所有検査1〜4を実装し、検査5の保存を専用architecture testのNix check出力へ接続した。検査1=`visible_revision_commit_has_single_owner`（`contract_sync`、`:2359-2487`）、検査2=`camera_shared_mapping_release_has_single_owner`、検査3=`serial_port_ownership_has_single_owner`、検査4=`public_wire_contracts_exclude_native_handles_and_private_paths`（`contract_sync`）＋`messagepack_extension_values_are_rejected_at_decode`（`worker/ipc/codec.rs`）。`contract_sync`全体20 passed／codec suite 8 passed（log `/tmp/ownership-checks-2319f15.log`）、各検査はbreak-red確認済み・context-separated review LGTM。検査5は`checks.rust-ci-core`（`rustCoreCheck`）と`checks.contract-sync`（`contractSyncCheck`）の`passed`出力として保存され、Normal CIのRust region jobが`nix run .#rust-ci-core`／`.#ci-rust-contracts`でrealizeする。local runの証跡でありcommit別CI証跡ではない）。
...
- [x] **AR-11-33** 各内部moduleが所有する責務を確定する（2026-09-27、`rust/pokecon/registry/ownership.json`（schema_version=1、§3.1の全12行→18エントリ。owner_symbolsはgrep実在確認済み、Rust symbol非所有のfrontend／generated_client／registryは空配列+noteで正直記録）と`contract_sync`の`module_ownership_manifest_pins_owner_symbols`（id双射・symbol実在・lib.rs crate-private pin・fail-closed制御）を実装。focused 1／full 21 passed、break-red 2種、context-separated review LGTM（`deleg_54f1dd2c`）。commit `2ac00b6`、Nix `checks.contract-sync` exit 0、commit別CI証跡: `4bc7a1f`のNormal／Package CI Required success（run `36266228925`／`36266228937`））。
- [x] **AR-11-34** 各内部moduleが所有しない責務を確定する（2026-09-27、ownership.json全18エントリへ`non_responsibilities`と13エントリの`forbidden_symbols`／`forbidden_scan_files`（必須3: server→CameraManager/SerialManager、application_backend→TcpListener、worker_supervisor→StateHub）を追加し、`contract_sync`の`forbidden_ownership_manifest_is_enforced`（不在・liveness・sentinel・§9.2全被覆を主張しない限定）を実装。focused 1／full 22 passed、break-red確認、review LGTM（`deleg_dcd709fb`）。commit `36e3159`（初回CI失敗はrustTestSourceへのownership.json追加`932ff90`で解消）、Nix `checks.contract-sync` exit 0、commit別CI証跡: `4bc7a1f`でsuccess）。
- [x] **AR-11-35** 許可する内部依存方向を確定する（2026-09-27、`rust/pokecon/registry/dependency_edges.json`（allowed 104 edge・schema_version=1）と`contract_sync`の`source_dependency_edges_match_allowed_manifest`（共有extractor・双方向diff・§9.1連動・scanned>100・sentinel）を実装。focused 1／full 23 passed、break-red 2種、review LGTM（`deleg_18931d04`）。commit `932ff90`、Nix `checks.contract-sync` exit 0、commit別CI証跡: `4bc7a1f`でsuccess）。
- [x] **AR-11-36** 禁止する内部依存方向を確定する（2026-09-27、dependency_edges.jsonへ`forbidden`27対（下位層→production/entrypoint 22・worker→device/server 2・device/camera/runtime→server 3。worker→cameraはwire値型のみのため除外をnote化）を追加し、`forbidden_dependency_edges_are_rejected`（実測∩禁止=∅・fixture positive control・sentinel・lib→production例外liveness・27対exact-pin）と`tests/fixtures/forbidden_dependency_fixture.rs`を実装。focused 1／full 24 passed、break-red 3種、review NEEDS_CHANGES→policy pin追加→LGTM（`deleg_8fe61737`）。commit `4bc7a1f`、Nix `checks.contract-sync` exit 0、commit別CI証跡: Normal／Package CI Required success（run `36266228925`／`36266228937`））。
...
- [ ] **AR-11-37** 別processと同一processの境界を確定する（予定証跡: process／module deployment diagramとIPC境界test）。
  - 実装済み: 境界を「Rust application processと2 worker OS process（script／dynamic）間のtyped MessagePack IPC」として`rust/pokecon/registry/ipc_boundary.json`（Envelope 5 kind・`IpcValue` 9値・上限1,048,576B・禁止wire marker 6・非公開payload 7ケース）と`contract_sync`の`worker_ipc_deployment_boundary_is_pinned`（exact-set・source drift・fixture liveness・sentinel・範囲限定comment）、`worker/ipc/codec.rs`の`fixture_nonpublic_wire_payloads_are_rejected_at_decode`（include! fixture、6 DecodeErr＋1 SchemaErr、positive control）で固定。focused 1／full 25 passed、break-red 2種、review LGTM（`deleg_417d9009`）、commit `fae2479`、Nix `checks.contract-sync`／`checks.rust-ci-core`／`checks.production-routing-audit` exit 0。初回CI `9a55506`のPython tests失敗はhandoff状態語彙の修正で解消（`nix run .#test`577 passed）。commit別CI証跡は修正commitで取得する。
...
- 2026-09-26 public boundary narrow evidence: `rust/pokecon/tests/contract_sync.rs`の`public_wire_contracts_exclude_native_handles_and_private_paths`が、`rust/pokecon/src/server/api.rs`、`rust/pokecon/registry/protocol.json`、`api/openapi.json`、`web/src/lib/api/openapi.ts`のnative handle／private object／Rust private path／dynamic callback ownership overclaim markerをfail-closedに検査する。focused testは1 passed、`contract_sync`全体は17 passed、negative predicateのbreak-redはexit 101だった。これはAR-11-27のsource-level narrow evidenceであり、runtime dynamic callback ownership、full IPC payload corpus、実browser／実device受入の完了根拠ではない。
- 2026-09-26 contract-check narrow evidence: HEAD `681c8ae`で`nix run .#contract-check`がexit 0（約8秒、`ok -- validation done`、openapi-typescript生成成功、logは`/tmp/contract-check-681c8ae.log`）。これはAR-13.1-03の単独gate初回証拠であり、全checkpoint分のlogとCI run URLが揃うまで完了根拠ではない。
- 2026-09-26 AR-11-25 camera narrow evidence: `rust/pokecon/tests/contract_sync.rs`の`camera_shared_mapping_release_has_single_owner`がcamera release marker（`shm_unlink`／`unlink_name`／`munmap`／`UnmapViewOfFile`）の所有を`rust/pokecon/src/camera/manager.rs`と`rust/pokecon/src/camera/shared_ring.rs`に限定し、production shutdown fallback（`retain_camera_fallback_on_timeout`／`camera_writer_fallback`／`UnstoppedCameraWriter`）の保持と非解放を検査する。focused testは1 passed、`contract_sync`全体は18 passed、fallback反転break-redはexit 101だった。これは5検査中1検査のnarrow evidenceであり、AR-11-25 checkboxの完了根拠ではない。
- 2026-09-27 checkpoint gate evidence: HEAD `10671bc`で、gate分離snapshot（実行ごとの`/tmp/pokecon-rust-gate-home.*`Cargo target）の`nix run .#cargo-test`がexit 0（223秒、workspace 542 passed／0 failed、contract_sync 18件。log `/tmp/gate-cargo-test-10671bc.log`）。同HEADで`nix run .#clippy` exit 0（126秒、警告0。log `/tmp/gate-clippy-10671bc.log`）、`nix run .#build-rust` exit 0（149秒。log `/tmp/gate-build-rust-10671bc.log`）。これはAR-13.1-04／05／06の単独gate初回証拠であり、全checkpoint分のlogとCI run URLが揃うまで完了根拠ではない。
- 2026-09-27 compatibility gate evidence: `nix run .#compatibility` exit 0（build 6m54s、`python -m scripts.compatibility.promote --check`と`runner --check`が完了し、`compatibility-report/1`は`"result":"passed"`、baseline 3／command 98／script 103、manifest SHA-256 `c2e4bd4ec3e0c5e3749e9e8ee84b046a477b8da6a2a160ec8a6121a7a461aeaf`、results SHA-256 `8443242348120cda654be48715b3e331b1c23831bda841cf1dc959cf6b275cd7`。log `/tmp/gate-compatibility-10671bc-rerun.log`）。これはAR-13.1-10の単独gate証拠であり、全checkpoint分のlogとCI run URLが揃うまで完了根拠ではない。
- 2026-09-27 AR-11-25 serial narrow evidence: `rust/pokecon/tests/contract_sync.rs`の`serial_port_ownership_has_single_owner`がnative serial port保持marker（`open_native_async`／`tokio_serial::new`／`SerialStream`）の存在を`rust/pokecon/src/device/serial/native.rs`に限定し、`SerialManager`（`manager.rs`）は`Arc<dyn SerialIo>`のtrait経由保持でmarkerを持たないこと、`selector.rs`の`available_ports`は列挙のみで除外されることを検査する。focused testは1 passed、`contract_sync`全体は19 passed、allowlist反転break-redはexit 101だった。これは重複所有検査#3のsource-level証拠であり、AR-11-25 checkboxの完了根拠ではない。
- 2026-09-27 AR-11-25 revision narrow evidence: `rust/pokecon/tests/contract_sync.rs:2359-2487`の`visible_revision_commit_has_single_owner`がvisible revision commit marker（`commit_projection`／`hub.commit`／`.commit(transaction)`）を`rust/pokecon/src/application_backend.rs`に限定し、test moduleがファイル末尾まで続く3ファイル（`server/state.rs`／`server/websocket.rs`／`server/rest/mod.rs`）は`#[cfg(test)]`でproduction prefixへ切詰めて走査する（切詰め前提はfail-closed assertで検証）。focused testは1 passed、`contract_sync`全体は20 passed、stale-allowlist break-redはFAILED確認後に復元。これは重複所有検査#1のsource-level証拠であり、AR-11-25 checkboxの完了根拠ではない。
- 2026-09-27 AR-11-25 IPC narrow evidence: `rust/pokecon/src/worker/ipc/codec.rs:333-363`の`messagepack_extension_values_are_rejected_at_decode`が、MessagePack拡張値（fixext4 timestamp）をpayload直下およびmap値ネストに置いたenvelopeの`decode_payload`が`CodecError::Decode`で拒否されることを検証する（envelopeのkind／opは既知valid値で、異なる部分はext値のみ）。focused testは1 passed、codec suiteは8 passed、break-redはFAILED確認後に復元。これは重複所有検査#4のworker IPC側runtime-negative証拠であり、AR-11-25 checkboxの完了根拠ではない。
- 2026-09-27 AR-11-25 completion evidence: 重複所有検査1〜4の実装（検査1〜3＝`contract_sync` source check、検査4＝`contract_sync` public boundary scan＋`worker/ipc/codec.rs` runtime-negative）と検査5の保存接続（`checks.rust-ci-core`／`checks.contract-sync`の`passed`出力をNormal CIのRust region jobがrealize）を確認し、`AR-11-25`を`[x]`化した。combined run log `/tmp/ownership-checks-2319f15.log`（`contract_sync` 20 passed／codec 8 passed）。残る`AR-10.8-03`はchange-path reviewのみ。
