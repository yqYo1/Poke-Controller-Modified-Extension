...
- `performance-check`は絶対閾値を毎回評価し、同じworkflow eventで全体がsuccessとなった直近最大5件の検証済みpassed reportをbaselineとして各metricのnearest-rank中央値を求めます。同じeventのsuccess reportが5件未満の場合はbaseline比較を行わずbootstrapとして記録し、relative regressionの成功とは扱いません。5件揃った場合、その中央値に対してlatency 10%増加またはFPS 5%低下をregression failureとします。同一対象commitの別runはbaselineへ再利用しません。過去reportがない初回はbootstrapと記録し、regression成功とは扱いません。
