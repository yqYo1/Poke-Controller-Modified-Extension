...
4. **動的設定の即時評価**（`init.py`/`init.lua`のトップレベルコード） — 非遅延トップレベル代入に限る
5. **通常CLI引数**（一般設定オプション） — 最優先・最後に適用

```
起動時評価パイプライン: ①デフォルト → ②グローバル → ③プロファイル → ④動的設定(即時) → ⑤通常CLI引数
                       （低）                                                       （高）
```

**ブートストラップCLI解析**:

上記パイプラインに先立ち、以下の目的に必要な最小限のCLI引数のみを事前解析する:

- 設定ソースの探索・選択（configファイルパス、プロファイル等）
- 動的設定実行環境の特定

このカテゴリには、config/profile/source/runtimeセレクターが含まれる（具体的なフラグ名はCLIスキーマ設計で別途決定する）。ブートストラップ解析で得られた値は、設定パイプライン全体の起点として使用される—後続のパイプライン適用より優先されるわけではなく、「何を読み込むか」を決定するための事前処理である。
