...
3. 動的設定にPython（`dynamic_config_language = "python"`）が選択された場合、アプリケーションは動的設定（`init.py`）実行専用の独立したOSプロセスを生成する。このプロセスは自身のCPythonメインインタープリターを持ち、ユーザースクリプトとは別に選択されたvenv/インタープリターを使用する。
4. 動的設定用Python実行環境が不要な場合（`dynamic_config_language`が`"python"`以外）、動的設定用Pythonプロセスは生成しない。
5. 分離手段としてCPythonサブインタープリターは採用しない。根拠: 単一のCPythonランタイムはただ一つのメインインタープリターを持ち、同一プロセス内に追加で生成できるのはサブインタープリターのみである。サブインタープリターはPyO3およびサードパーティ拡張との互換性が保証されないため、必要な分離を得るために別プロセス方式を採用する。
6. プロセス境界により、`sys.path`、インポート済みモジュール、グローバル状態、Pythonオブジェクトは完全に分離される。Pythonオブジェクトがプロセス境界を越えることはない。
7. 動的設定からユーザー向け `pokecon.*` APIへのアクセスはRustが管理する内部IPC/API境界を経由する。この内部形式は実装詳細であり、公開APIの名前と動作を変更しない限りユーザーに露出しない。
8. PythonとLuaの動的設定APIは公開APIレベルで同一の動作を提供する。IPC方式の違いは公開APIの動作に影響しない。
...
**注**: ユーザースクリプトや動的設定（`init.py`/`init.lua`）から呼び出されるAPIは、原則としてPyO3（Rust製）で実装される。Pythonファイル（`commands.py`, `events.py`等）は型注釈・ドキュメント・互換レイヤーのみを提供し、実際の処理はRust側で行う。ユーザースクリプト用PythonはRustメインプロセスにPyO3で直接埋め込まれるため、Python→RustのAPI呼び出しは同一プロセス内の直接的な関数呼び出しとして行われる。動的設定用Pythonは別プロセスで動作するため、`pokecon.*` API呼び出しはRust管理の内部IPCを経由するが、公開APIの名前と動作は同一である。これらの内部境界の詳細はユーザー向けAPIに露出しない。
...
- **venv**: いずれの環境でも、venvを作成して使用する。ユーザースクリプト用（Rustメインプロセス埋め込み）と動的設定用（別プロセス）で別々のvenvを使用可能
