...
現在のRust版が生成した設定fileと、将来のRust版が読み込む設定fileの互換性は、この文書で別に定める設定契約です。

Rust版以降の設定loaderは、設定fileの内容を次の分類で扱います。

- **既知key**：正準registryの型と制約を検証し、値を適用します。
- **移動または廃止済みのkey**：registryへ明示的に登録された場合だけ、移行先または廃止理由を示す警告を出します。
- **無関係な未知key**：本体の起動を妨げず、設定fileに残したまま無視したことを警告します。
- **動的設定またはextensionが所有するkey**：本体の正準設定として解釈せず、所有者が読み取れるように保持します。
- **既知keyの不正値**：安全なregistry defaultへ置換できる場合は警告して継続します。安全な値を決められない場合だけ起動を停止します。

未知keyを理由に起動を停止すること、未知keyを読み込み時に削除すること、prototypeのkeyを暗黙に現行Rust keyへ変換することは、この契約に含まれません。

起動を停止する設定errorは、設定fileのpath、key、問題の値（secretは`[REDACTED]`）、期待する形式、停止理由、修正方法を利用者向けの文面で示します。

この分類はTOMLだけでなく、将来同じ設定registryを利用するCLI、環境変数、動的設定の診断にも適用します。

設定registryの`invalid_toml_value`は、既知TOML値を不正と判定した場合の安全性契約です。各設定に必ず明示します。

`use_default`は安全性を確認した設定だけに指定し、registryの既定値へ戻して継続します。`reject`は既定値へ戻すと意味または安全性を損なうため停止します。ポリシーを省略したregistry項目は安全側の`reject`として扱います。
