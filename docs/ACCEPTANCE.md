...
- `performance-check`は絶対閾値を毎回評価し、同じworkflow event・同じbranch（PRは同じPR）・同じfixture世代で全体がsuccessとなった直近最大5件の検証済みpassed reportをbaselineとして各metricを集約します。browser loopback fixture本体（`scripts/performance/`）またはruntime入力（`flake.nix`、`flake.lock`）が変更されたrunでは、latencyの相対許容上限を`max(中央値×1.10, 履歴p95最大値)`、FPSの相対許容下限を`min(中央値×0.95, 履歴p50最小値)`としてregressionを判定します。これは履歴の実測分散内を正常変動として扱うためで、絶対閾値を緩めるものではありません。本番Rust／Web UI／serial等の変更はこのloopback fixtureを実行時に呼び出さないため、browser smokeと絶対閾値は実行しますが、relative baseline比較は行いません。同一対象commitの別runはbaselineへ再利用しません。過去reportがない初回はbootstrapと記録し、regression成功とは扱いません。
